	...
	for _, first := range testInParsed {
		for _, second := range testInParsed {
			if first+second == 2020 {
				utils.Log("info", "Part 1: Found", first, second, "\tres:", first*second)
			}
			for _, third := range testInParsed {
				if first+second+third == 2020 {
					utils.Log("warn", "Part 2: Found", first, second, third, "\tres:", first*second*third)
				}
			}
		}
	}
...
func main() {
	...
	utils.Log("info", "Solving test input")
	day1(testInput)
	utils.Log("info", "Solving real input")
	day1(realInput)
}
